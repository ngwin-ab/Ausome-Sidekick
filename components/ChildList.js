...
const ChildList = ({ navigation, deleteMode, onChildCountChange }) => {
    ...
    const Item = ({ item, onPress, handleEvent, deleteKid, disabled }) => (
        <TouchableOpacity onPress={deleteMode ? null : onPress} style={styles.item} >
            ...
            <View style={{ flex: 1 }}>
                <Text style={{ fontWeight: 'bold', fontSize: 20, color: '#051441' }}>{item.name}</Text>
                <Text style={{ fontSize: 16, color: '#051441' }}>Like: {item.like}</Text>
            </View>
            {deleteMode && (
            ...
            {data.length === 0 ? (
                ...
                <View>
                    ...
                    <TouchableOpacity
                        style={styles.addbox}
                        onPress={() => navigation.navigate('AddChild')}>
                        <Ionicons name='add-circle' size={50} color='#051441' />
                        <Text style={styles.heading}>Add child</Text>
                    </TouchableOpacity>
                </View>
            ) : (
